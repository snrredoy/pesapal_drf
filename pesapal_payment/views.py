...
from .pesapal_service import submit_order, get_transaction_status, register_ipn, submit_order_recurring
...
class CreateRecurringOrderView(APIView):
    ...
    def post(self, request):
        ...
        merchant_reference = f"subrec_{user.id}_{uuid.uuid4().hex[:12]}"
        notification_id = settings.PESAPAL_IPN_ID
        description = f"Recurring subscription {plan.name} for user {user.id}"

        start_date = timezone.now().strftime("%d-%m-%Y")
        end_date = (timezone.now() + timezone.timedelta(days=plan.duration_days)).strftime("%d-%m-%Y")

        resp = submit_order_recurring(
            merchant_reference=merchant_reference,
            amount=plan.price,
            email=user.email or "",
            phone="",
        ...
        checkout_url = resp.get("redirect_url") or resp.get("url")
...
class PesapalIPNView(APIView):
    ...
    def post(self, request):
        ...
        order_tracking_id = payload.get("order_tracking_id") or payload.get("orderTrackingId")
        merchant_ref = payload.get("merchant_reference") or payload.get("merchantReference")
        ...
        tx_status = status_resp.get("status")
        subscription_info = status_resp.get("subscription_transaction_info")  # recurring info
        ...
        if tx_status and tx_status.upper() == "COMPLETED":
            ...
            Subscription.objects.create(
                ...
                end_at=end_at,
                active=True,
                pesapal_correlation_id=subscription_info.get("correlation_id") if subscription_info else None,
                frequency=subscription_info.get("frequency") if subscription_info else order.plan.duration_days,
                next_payment_date=None  # you can calculate next payment from frequency
            )
